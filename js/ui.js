...
async function addStudyset(studyset) {
  ...
  // Convert the image file to a Data URL and store it in IndexedDB
  const imageDataURL = await convertFileToDataURL(studyset.image);

  if (navigator.onLine) {
    // Online - Add metadata only (no image upload to Firebase)
    const studysetToStore = {
      title: studyset.title,
      status: studyset.status,
    };

    // Save metadata to Firebase (without the image)
    const savedStudyset = await addStudysetToFirebase(studysetToStore);
    studysetId = savedStudyset.id;

    // Add studyset with Firebase ID and image Data URL to IndexedDB
    ...
    // Add studyset with image Data URL to IndexedDB
    const studysetToStore = {
      ...studyset,
      id: studysetId,
      image: imageDataURL,
    ...
    // Add studyset to store
    await store.add(studysetToStore);
  ...
  // Return studyset with ID and image data (stored in IndexedDB)
  return { ...studyset, id: studysetId, image: imageDataURL };
}

// Sync unsynced studysets from IndexedDB to Firebase
async function syncStudysets() {
  const db = await createDB();
...
async function loadStudysets() {
  ...
  studysets.forEach((studyset) => {
    // Ensure to handle the images properly when displaying them
...
function displayStudyset(studyset) {
  ...
  const deleteButton = studysetContainer.querySelector(
    `[data-id="${studyset.id}"] .studyset-delete`
  );
  deleteButton.addEventListener("click", () => deleteStudyset(studyset.id));

  // Attach edit event listener
  const editButton = studysetContainer.querySelector(
    `[data-id="${studyset.id}"] .studyset-edit`
  );
